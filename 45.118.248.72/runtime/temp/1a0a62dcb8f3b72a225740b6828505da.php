<?php /*a:2:{s:72:"/www/wwwroot/45.118.248.72/application/admin/view/help/edit_message.html";i:1586093112;s:59:"/www/wwwroot/45.118.248.72/application/admin/view/main.html";i:1593205496;}*/ ?>
...
require(['ckeditor', 'angular'], function() {
    ...
    function callback($rootScope) {
        ...
        $rootScope.batchSet = function(type, fixed) {
            layer.prompt({ title: '请输入数值', formType: 0 }, function(value, index) {
                $rootScope.$apply(function() {
                    ...
                    for (var i in $rootScope.specsTreeData)
                        for (var j in $rootScope.specsTreeData[i]) {
        ...
        $rootScope.$watch('specs', function() {
            ...
            for (var i in data) {
                ...
                for (var j in data[i].list)
                    if (data[i].list[j].check && data[i].list[j].name.length > 0) {
            ...
            for (var i in list) {
                var tmp = [];
                for (var j in table)
                    for (var k in list[i]) tmp.push(table[j].concat(list[i][k]));
                table = tmp;
            }
            // 表格合并
        ...
        $rootScope.delSpecRow = function(data, $index) {
            var tmp = [];
            for (var i in data)
                if (parseInt(i) !== parseInt($index)) tmp.push(data[i]);
            return $rootScope.specs = tmp;
        };
        // 增加分组的属性
        ...
        $rootScope.delSpecVal = function(data, $index) {
            var temp = [];
            for (var i in data)
                if (parseInt(i) !== parseInt($index)) temp.push(data[i]);
            return temp;
        };
    }
})
